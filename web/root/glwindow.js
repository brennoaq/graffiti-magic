...
class GLWindow {
  ...
  #handleMouseMove(e) {
    ...
    const pos = this.click({ x: x, y: y });

    let canvasX = pos.x;
    let canvasY = pos.y;

    if (this.#prevHover.color) {
      this.setPixelColor(
        this.#prevHover.x,
        this.#prevHover.y,
        this.#prevHover.color
      );
    }

    this.#prevHover.x = Math.floor(canvasX);
    this.#prevHover.y = Math.floor(canvasY);
    this.#prevHover.color = this.getColor({
      x: this.#prevHover.x,
      y: this.#prevHover.y,
    });

    this.setPixelColor(this.#prevHover.x, this.#prevHover.y, [209, 209, 209]);
    this.draw();
  }
  ...
  draw(clean = false) {
    if (clean) {
      this.#prevHover = { x: -1, y: -1, color: null };
    }
  ...
  setPixelColor(x, y, color) {
    this.#gl.bindTexture(this.#gl.TEXTURE_2D, this.#tex); // Bind the texture
    let rgba = new Uint8Array(4);
    rgba[3] = 255; // Assuming the color is in RGBA format
  ...
  getColor(pos) {
    ...
    this.#gl.bindFramebuffer(this.#gl.FRAMEBUFFER, null); // Unbind the framebuffer after reading pixels
    return rgba.slice(0, 3); // Return RGB values
  }
