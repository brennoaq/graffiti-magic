...
func init() {
	...
	flag.IntVar(&width, "width", 300, "The width to create the canvas.")
	flag.IntVar(&height, "height", 300, "The height to create the canvas.")
	...
	flag.BoolVar(&enableWL, "wl", true, "Enable whitelist.")
}
...
func main() {
	...
	var img draw.Image
	if loadPath == "" {
		log.Printf("[main] Creating new canvas with dimensions %d x %d\n", width, height)
	...
	if enableWL {
		...
		whitelist = d
		if loadRecordPath == "" {
			log.Printf("[main] Creating new record image with dimensions 2 %d x %d\n", width, height)
